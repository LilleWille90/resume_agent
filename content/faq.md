# FAQ (Recruiter-friendly)

## Can you summarise your background in 30 seconds?
I’m a Product Manager with 8+ years across consumer apps, IoT and internal tools. Today I own strategy and roadmap for a domain in Electrolux OneApp and lead cross-functional teams across mobile, backend, firmware, data, UX and compliance functions.

## What kinds of products do you enjoy most?
Products with real user impact and meaningful complexity: platforms, data/integrations, multi-market experiences, and connected products where software increases the value of hardware.

## What’s a concrete example of impact?
We built pollen-based automation for connected air purifiers by combining external pollen data, geolocation/forecasts, and on-device sensor data. We shipped within two sprints and saw 25%+ of active users onboard and keep it running continuously.

## How do you work with data?
I use funnels, cohorts and usage analytics to prioritise, measure impact, and iterate. I pair quantitative signals with qualitative insights to avoid “metric-only” decisions.

## How would colleagues describe you?
Social, open-minded, enthusiastic people person; solution-oriented with a creative edge.

## What’s your AI experience?
I’ve been building agents/apps myself and I’m motivated by trustworthy AI that integrates deeply with existing systems, iterates fast, and keeps quality high. (For work examples: see my product cases and how I’ve used data/integrations in shipped features.)

## What do you want to be asked in an interview?
- “Walk me through a product decision you changed your mind on and why.”
- “Show a case where you shipped fast but kept quality high.”
- “How do you align stakeholders in complex organisations?”
